...
function ContestCard({ contest, onEnter }: { contest: Contest; onEnter: (c: Contest) => void }) {
  ...
  const fill = Math.min((contest.currentEntries / contest.maxEntries) * 100, 100);
  ...
  return (
    ...
    >
      <div className="card-inner p-6 flex flex-col gap-4 h-full">
        ...
        <div>
          <h3 className="font-bold text-base leading-tight mb-1">{contest.name}</h3>
          <p className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>
            {contest.description || `${contest.leagueType} matchday ${contest.matchday} contest`}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: "Entry", value: contest.entryFee === 0 ? "FREE" : `${contest.entryFee.toLocaleString()} FC`, highlight: true },
            { label: "Prize Pool", value: `${contest.prizePool.toLocaleString()} FC`, highlight: false },
            { label: "Spots Left", value: spotsLeft.toLocaleString(), highlight: false },
        ...
        >
          {spotsLeft <= 0 ? "Full" : contest.entryFee === 0 ? "Join Free" : `Join · ${contest.entryFee} FC`}
...
export default function ContestsPage() {
  ...
  const load = useCallback(async (type = "all") => {
    ...
    const data = await fetch(url).then((r) => r.json()).catch(() => ({ contests: [] }));
        ...
        <div className="max-w-6xl mx-auto px-4 py-12">
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {FILTERS.map((f) => (
              <button
                ...
                style={{
                  background: filter === f ? "var(--neon)" : "rgba(255,255,255,0.05)",
                  color: filter === f ? "#000" : "rgba(255,255,255,0.6)",
                  border: "1px solid",
                  borderColor: filter === f ? "var(--neon)" : "rgba(255,255,255,0.08)",
                  transitionTimingFunction: "var(--spring)",
                }}
              >
          ...
          {msg && (
            <div
              ...
              style={{
                background: msg.ok ? "rgba(0,255,135,0.1)" : "rgba(239,68,68,0.1)",
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="skeleton" style={{ height: "320px", borderRadius: "1.5rem" }} />
              ))}
            </div>
          ) : contests.length === 0 ? (
            <div className="text-center py-32" style={{ color: "rgba(255,255,255,0.3)" }}>
          ...
          <div className="card-outer w-full max-w-md">
            <div className="card-inner p-8">
              ...
              <div className="flex gap-3">
                <button onClick={() => setEntering(null)} className="btn-ghost flex-1 justify-center">
                  Cancel
                </button>
                <button onClick={handleEnter} className="btn-neon flex-1 justify-center">
                  Confirm Entry
