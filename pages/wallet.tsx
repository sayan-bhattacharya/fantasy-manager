...
const TX_COLORS: Record<string, string> = {
  deposit: "#22c55e", withdrawal: "#ef4444", contest_entry: "#f59e0b",
  contest_win: "#00FF87", bonus: "#a78bfa",
};
...
export default function WalletPage() {
  ...
  const handleAction = async () => {
    ...
    if (r.ok) {
      setMsg({ text: action === "deposit" ? `Deposited ${amount} FC` : `Withdrew ${amount} FC`, ok: true });
      setData((prev) => prev ? {
        ...prev,
        wallet: { ...prev.wallet, balance: d.balance },
        transactions: [
          { id: Date.now(), amount: action === "deposit" ? parseFloat(amount) : -parseFloat(amount), type: action, description: action, createdAt: Math.floor(Date.now() / 1000) },
          ...prev.transactions,
        ],
      } : prev);
    } else {
  ...
  return (
    <>
      <Head><title>Wallet — FantasyKick</title></Head>
      <Menu />
      ...
      <main className="main-content" style={{ background: "var(--bg)" }}>
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(0,255,135,0.07) 0%, transparent 70%)", borderBottom: "1px solid var(--border)" }}
        >
        ...
        <div className="max-w-4xl mx-auto px-4 py-12">
          {loading ? (
            <div className="space-y-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="skeleton" style={{ height: "100px", borderRadius: "1.5rem" }} />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div
                ...
                style={{ background: "linear-gradient(135deg, rgba(0,255,135,0.06) 0%, rgba(0,229,255,0.04) 100%)" }}
              >
                <div className="card-inner p-8 text-center">
                  ...
                  <div className="wallet-balance mb-1">{data?.wallet.balance.toLocaleString()}</div>
                  <div className="text-sm font-bold" style={{ color: "var(--neon)" }}>Fantasy Coins</div>
                  ...
                  <div className="mt-8 space-y-3">
                      ...
                      {(["deposit", "withdraw"] as const).map((a) => (
                        <button
                          ...
                          style={{
                            ...
                            color: action === a ? "#000" : "rgba(255,255,255,0.5)",
                    ...
                    <div className="flex flex-wrap gap-1.5 justify-center">
                      {QUICK.map((q) => (
                        <button
                          ...
                          style={{
                            background: amount === String(q) ? "rgba(0,255,135,0.2)" : "rgba(255,255,255,0.05)",
                            color: amount === String(q) ? "var(--neon)" : "rgba(255,255,255,0.5)",
                    ...
                    <button onClick={handleAction} className="btn-neon w-full justify-center" style={{ padding: "0.65rem 1rem", fontSize: "0.85rem" }}>
                      {action === "deposit" ? "Add Coins" : "Withdraw"}
                      <span className="btn-icon">{action === "deposit" ? "+" : "-"}</span>
                    </button>

                    {msg && (
                      <p className="text-xs font-semibold" style={{ color: msg.ok ? "var(--neon)" : "#ef4444" }}>
                        {msg.text}
                      </p>
                    )}
                  </div>
                      ...
                      (data?.transactions ?? []).map((tx) => (
                        ...
                        >
                          ...
                          <div
                            ...
                            style={{ color: TX_COLORS[tx.type] ?? (tx.amount > 0 ? "#22c55e" : "#ef4444") }}
                          >
