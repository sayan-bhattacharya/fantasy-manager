...
// ─── Player definitions ───────────────────────────────────────────────────────
const DEFS = [
  { id: 0,  role: "gk",  hx: 150, hy: 46,  range: 16, color: "#FBBF24", glow: "rgba(251,191,36,0.7)"  },
  { id: 1,  role: "def", hx: 80,  hy: 100, range: 38, color: "#3B82F6", glow: "rgba(59,130,246,0.7)"  },
  { id: 2,  role: "def", hx: 148, hy: 92,  range: 38, color: "#3B82F6", glow: "rgba(59,130,246,0.7)"  },
  { id: 3,  role: "def", hx: 218, hy: 100, range: 38, color: "#3B82F6", glow: "rgba(59,130,246,0.7)"  },
  { id: 4,  role: "mid", hx: 52,  hy: 162, range: 60, color: "#A855F7", glow: "rgba(168,85,247,0.7)"  },
  { id: 5,  role: "mid", hx: 120, hy: 152, range: 60, color: "#A855F7", glow: "rgba(168,85,247,0.7)"  },
  { id: 6,  role: "mid", hx: 182, hy: 152, range: 60, color: "#A855F7", glow: "rgba(168,85,247,0.7)"  },
  { id: 7,  role: "mid", hx: 248, hy: 162, range: 60, color: "#A855F7", glow: "rgba(168,85,247,0.7)"  },
  { id: 8,  role: "fwd", hx: 88,  hy: 226, range: 54, color: "#EF4444", glow: "rgba(239,68,68,0.7)"   },
  { id: 9,  role: "fwd", hx: 150, hy: 234, range: 54, color: "#EF4444", glow: "rgba(239,68,68,0.7)"   },
  { id: 10, role: "fwd", hx: 212, hy: 226, range: 54, color: "#EF4444", glow: "rgba(239,68,68,0.7)"   },
] as const;
...
interface Player extends Omit<Def, "range"> {
  x: number; y: number;
  px: number; py: number; // perlin offset phases
...
function buildAudio() {
  try {
    ...
    bp.type = "bandpass"; bp.frequency.value = 680; bp.Q.value = 0.45;

    // Low-shelf — stadium rumble
    const ls = ctx.createBiquadFilter();
    ls.type = "lowshelf"; ls.frequency.value = 220; ls.gain.value = 9;

    // High-shelf rolloff — take harsh edge off
    const hs = ctx.createBiquadFilter();
    hs.type = "highshelf"; hs.frequency.value = 3500; hs.gain.value = -8;

    src.connect(bp); bp.connect(ls); ls.connect(hs); hs.connect(master);
    src.start();
    ...
    return {
      ...
      cheer(intensity = 1.0) {
        ...
        master.gain.linearRampToValueAtTime(Math.min(0.6 * intensity, 0.6), now + 0.35);
...
function bezier(t: number, p0x: number, p0y: number, p1x: number, p1y: number, p2x: number, p2y: number) {
  const mt = 1 - t;
  return {
...
const rng = (lo: number, hi: number) => Math.floor(Math.random() * (hi - lo) + lo);
...
export default function AnimatedPitch() {
  ...
  useEffect(() => {
    ...
    const pickPass = (fromId: number, toId: number | null = null) => {
      ...
      if (!longBall) {
        ...
        candidates = candidates.filter(d => Math.abs(order.indexOf(d.role) - ri) <= 1);
      }
      if (candidates.length === 0) candidates = DEFS.filter(d => d.id !== fromId);
      const pick = toId !== null ? DEFS[toId] : candidates[rng(0, candidates.length)];
      return pick.id;
    };
    ...
    const drawPitch = (sc: number) => {
      ...
      for (let i = 0; i < 6; i++) {
        ctx2.fillStyle = i % 2 === 0 ? "rgba(0,40,0,0.35)" : "rgba(0,30,0,0.35)";
      ...
      // Halfway line
      ctx2.beginPath(); ctx2.moveTo(10, S/2); ctx2.lineTo(S-10, S/2); ctx2.stroke();
      // Centre circle
      ctx2.beginPath(); ctx2.arc(S/2, S/2, 38, 0, Math.PI*2); ctx2.stroke();
      // Centre dot
      ctx2.fillStyle = "rgba(0,255,135,0.5)";
      ctx2.beginPath(); ctx2.arc(S/2, S/2, 2.5, 0, Math.PI*2); ctx2.fill();
      // Penalty areas
    ...
    const drawPlayers = (sc: number) => {
      ...
      players.forEach(p => {
        ...
        ctx2.fillStyle = grad;
        ctx2.beginPath(); ctx2.arc(p.x, p.y, glowR, 0, Math.PI*2); ctx2.fill();

        // Player dot
        ctx2.fillStyle = p.color;
        ctx2.shadowColor = p.glow;
        ctx2.shadowBlur  = hasball ? 10 : 5;
        ctx2.beginPath(); ctx2.arc(p.x, p.y, radius, 0, Math.PI*2); ctx2.fill();
        ctx2.shadowBlur = 0;
        ...
        if (hasball) {
          ...
          ctx2.lineWidth   = 1.2;
          ctx2.beginPath(); ctx2.arc(p.x, p.y, 8.5 + pulse * 3, 0, Math.PI*2); ctx2.stroke();
          ctx2.globalAlpha = 1;
        }
      });
    ...
    const drawBall = () => {
      ...
      ball.trail.forEach((pt, i) => {
        ...
        ctx2.beginPath(); ctx2.arc(pt.x, pt.y, 2 + frac * 1.5, 0, Math.PI*2); ctx2.fill();
      });
      ctx2.globalAlpha = 1;

      // Ball glow
      const bg = ctx2.createRadialGradient(ball.x, ball.y, 0, ball.x, ball.y, 12);
      bg.addColorStop(0, "rgba(255,255,255,0.4)");
      bg.addColorStop(1, "rgba(0,0,0,0)");
      ctx2.fillStyle = bg;
      ctx2.beginPath(); ctx2.arc(ball.x, ball.y, 12, 0, Math.PI*2); ctx2.fill();

      // Ball body
      ctx2.fillStyle = "#ffffff";
      ctx2.shadowColor = "rgba(255,255,255,0.9)";
      ctx2.shadowBlur  = 8;
      ctx2.beginPath(); ctx2.arc(ball.x, ball.y, 3.5, 0, Math.PI*2); ctx2.fill();
      ctx2.shadowBlur  = 0;

      // Flash on shot/celebration
      if (ball.flashAlpha > 0.01) {
        const fg = ctx2.createRadialGradient(ball.x, ball.y, 0, ball.x, ball.y, 60 * ball.flashAlpha);
        fg.addColorStop(0, `rgba(255,220,50,${ball.flashAlpha * 0.8})`);
        fg.addColorStop(0.4, `rgba(255,140,0,${ball.flashAlpha * 0.3})`);
        fg.addColorStop(1, "rgba(0,0,0,0)");
        ctx2.fillStyle = fg;
        ctx2.beginPath(); ctx2.arc(ball.x, ball.y, 60, 0, Math.PI*2); ctx2.fill();
    ...
    const tick = () => {
      ...
      switch (ball.state) {
        ...
        case "passing": {
          ...
          const pos = bezier(et, ball.fromX, ball.fromY, ball.cpX, ball.cpY, ball.toX, ball.toY);
          // Add trail
          ...
          if (ball.t >= 1) {
            ...
            let nearest = 0, bestD = 99999;
            players.forEach(p => {
              const d = Math.hypot(p.x - ball.x, p.y - ball.y);
              if (d < bestD) { bestD = d; nearest = p.id; }
            });
  ...
  return (
    <div style={{ position: "relative", width: "min(340px, 90%)", aspectRatio: "1" }}>
      {/* Canvas pitch */}
      <canvas
        ref={canvasRef}
        style={{
      ...
      >
        ...
        />
        <span style={{ fontSize:"0.6rem", fontWeight:700, letterSpacing:"0.12em", color:"#fff", textTransform:"uppercase" }}>
          Live
        </span>
      </div>
